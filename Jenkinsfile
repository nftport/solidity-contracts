    ...
    stage('Mythx Analyze Quick') {
      ...
      steps {
            ...
            failureDescription: 'Mythx Analyze failed')
        {
          ...
          mythx --yes analyze --mode quick contracts --remap-import "@openzeppelin/=$(pwd)/node_modules/@openzeppelin/" --swc-blacklist SWC-123 --solc-version 0.8.1
          '''
        }
      }
    }
    stage('Mythx Analyze Standard') {
      ...
      steps {
            ...
            failureDescription: 'Mythx Analyze failed')
        {
          sh '''
          source activate $(head -1 environment.yml | cut -d' ' -f2)
          mythx --yes analyze --mode standard contracts --remap-import "@openzeppelin/=$(pwd)/node_modules/@openzeppelin/" --swc-blacklist SWC-123 --solc-version 0.8.1
          '''
        }
      }
    }
